...
class StudentService:
    ...
    async def start_assessment(self, user_id: int, assessment_data: Dict[str, Any]) -> Assessment:
        ...
        try:
            ...
            # 从user_id获取实际的student_id - 使用直接SQL查询
            from sqlalchemy import text
            result = self.db.execute(text("""
                SELECT id, user_id, student_id, name, major, grade, phone, emergency_contact, emergency_phone
                FROM students 
                WHERE user_id = :user_id
            """), {"user_id": user_id})
            student_data = result.fetchone()
            logger.debug(f"查询学生信息: student_data={student_data}")
            ...
            class SimpleStudent:
                def __init__(self, data):
                    ...
                    self.emergency_phone = data.emergency_phone
            
            student = SimpleStudent(student_data)
