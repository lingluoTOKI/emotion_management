    ...
    def _intelligent_emotion_mapping(self, message: str, bert_emotion: str, confidence: float) -> str:
        ...
        if detected_emotions:
            ...
            if max_score >= 1.5:  # 至少包含一个主要关键词或多个次要关键词
                logger.info(f"🎯 关键词明确识别情绪: {explicit_emotion} (得分: {max_score})，覆盖BERT分类: {bert_emotion}")
                return explicit_emotion
        
        # 3. 如果没有明确的关键词，基于BERT的基础分类进行合理映射
        logger.info(f"📊 关键词检测结果: {detected_emotions}")
        logger.info(f"🔄 BERT基础分类: {bert_emotion}，进行智能映射...")
        ...
        if bert_emotion == 'positive':
            ...
            final_emotion = "happiness"  # 默认积极情绪
            logger.info(f"✅ 积极情绪映射结果: {final_emotion}")
            ...
            if detected_emotions:
                # 有检测到情绪关键词，使用得分最高的
                final_emotion = max(detected_emotions, key=detected_emotions.get)
                logger.info(f"✅ 消极情绪细分结果: {final_emotion} (基于关键词)")
                return final_emotion
            else:
                # 没有明确关键词，根据消极语境推断
                # 检查是否包含压力、困扰等通用消极词汇
                ...
                if any(word in message_lower for word in stress_indicators):
                    final_emotion = "anxiety"  # 倾向于焦虑
                    logger.info(f"✅ 消极情绪推断结果: {final_emotion} (检测到压力指标)")
                    return final_emotion
                else:
                    final_emotion = "sadness"  # 默认为悲伤
                    logger.info(f"✅ 消极情绪默认结果: {final_emotion}")
                    return final_emotion
            ...
            if detected_emotions:
                ...
                final_emotion = max(detected_emotions, key=detected_emotions.get)
                logger.info(f"✅ 中性情绪修正结果: {final_emotion} (检测到明确情绪)")
                return final_emotion
            logger.info(f"✅ 保持中性情绪: neutral")
            return "neutral"
