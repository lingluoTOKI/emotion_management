...
class AICounselingSession(Base):
    ...
    session_type = Column(String(50))  # 会话类型：text/voice
    start_time = Column(DateTime(timezone=True), server_default=func.now())
    end_time = Column(DateTime(timezone=True))
    status = Column(String(50))  # 会话状态
...
class RiskAssessment(Base):
    ...
    intervention_required = Column(Boolean)  # 是否需要干预
    counselor_notified = Column(Boolean)     # 是否已通知咨询师
