...
def insert_sample_data():
    ...
    try:
        ...
        from app.models.user import User, Admin, Student, Counselor, UserRole, CounselorSchool
        from app.models.assessment import Assessment, AssessmentRecord, EmotionRecord, AssessmentType
        from app.models.consultation import Consultation, ConsultationRecord, Appointment, ConsultationType, ConsultationStatus
        from app.models.ai_counseling import AICounselingSession, RiskAssessment
        from app.models.anonymous import AnonymousMessage, AnonymousChat
        # 使用明文密码，不需要密码哈希
        ...
        if existing_users > 0:
            logger.info(f"数据库中已有 {existing_users} 个用户，清理旧数据...")
            
            # 清理所有相关数据（按依赖关系顺序删除）
            # 先删除有外键依赖的表
            db.query(AnonymousMessage).delete()
            db.query(AICounselingSession).delete()
            db.query(Consultation).delete()
            db.query(Assessment).delete()
            db.query(Student).delete()
            db.query(Counselor).delete()
            db.query(Admin).delete()
            db.query(User).delete()
            
            # 清理所有表数据（使用原生SQL避免外键约束）
            db.execute(text("SET FOREIGN_KEY_CHECKS = 0"))
            db.execute(text("TRUNCATE TABLE anonymous_messages"))
            db.execute(text("TRUNCATE TABLE anonymous_chats"))
            db.execute(text("TRUNCATE TABLE risk_assessments"))
            db.execute(text("TRUNCATE TABLE ai_counseling_sessions"))
            db.execute(text("TRUNCATE TABLE consultation_records"))
            db.execute(text("TRUNCATE TABLE consultations"))
            db.execute(text("TRUNCATE TABLE appointments"))
            db.execute(text("TRUNCATE TABLE emotion_records"))
            db.execute(text("TRUNCATE TABLE assessment_records"))
            db.execute(text("TRUNCATE TABLE assessments"))
            db.execute(text("TRUNCATE TABLE students"))
            db.execute(text("TRUNCATE TABLE counselors"))
            db.execute(text("TRUNCATE TABLE admins"))
            db.execute(text("TRUNCATE TABLE users"))
            db.execute(text("SET FOREIGN_KEY_CHECKS = 1"))
            
            db.commit()
            logger.info("旧数据清理完成，开始插入新数据")
