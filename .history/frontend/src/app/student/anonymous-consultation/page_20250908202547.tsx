...
export default function AnonymousConsultation() {
  ...
  const analyzeMessageWithEasyBert = async (message: string): Promise<{
    ...
    try {
      ...
      console.log('🧠 EasyBert情感分析结果:', response)
      
      // 提取风险等级和情感分析结果
      const risk_level = response.risk_assessment?.risk_level || 'low'
      const emotion_analysis = response.emotion_analysis
      
      return {
        risk_level: risk_level as 'minimal' | 'low' | 'medium' | 'high',
        emotion_analysis,
        confidence: emotion_analysis?.confidence || 0.5
      }
    } catch (error) {
      console.error('❌ EasyBert情感分析失败:', error)
      // 如果API调用失败，回退到关键词检测
      return {
        risk_level: detectRiskLevel(message),
        confidence: 0.3
      }
    }
  }
