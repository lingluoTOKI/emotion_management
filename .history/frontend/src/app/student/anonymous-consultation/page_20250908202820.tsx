...
interface AnonymousMessage {
  ...
  isRead: boolean
  emotionAnalysis?: any  // EasyBert情感分析结果
  riskAssessment?: any   // EasyBert风险评估结果
}
...
const positiveKeywords = [
  '开心', '快乐', '高兴', '愉快', '满足', '幸福', '舒服', '放松', '轻松', '平静',
  '感觉很好', '感觉不错', '非常好', '很棒', '满意', '充实', '有希望', '积极',
  '旅游', '游玩', '享受', '喜欢', '爱好', '兴趣', '期待', '向往', '美好'
]
...
export default function AnonymousConsultation() {
  ...
  const [aiSessionId, setAiSessionId] = useState<string | null>(null)  // AI会话ID
  ...
  // 使用EasyBert进行情感分析和风险评估
  const analyzeMessageWithEasyBert = async (message: string): Promise<{
    riskLevel: 'minimal' | 'low' | 'medium' | 'high'
    emotionAnalysis?: any
    riskAssessment?: any
  }> => {
    try {
      ...
      if (!sessionId) {
        ...
        console.log('✅ 匿名咨询AI会话创建成功:', sessionId)
      }

      // 调用AI分析API获取EasyBert结果
      console.log('🧠 发送消息到EasyBert分析，会话ID:', sessionId)
      const analysisResult = await api.ai.chat({ 
        session_id: sessionId, 
        message: message 
      })
      
      console.log('📊 EasyBert分析结果:', JSON.stringify(analysisResult, null, 2))
      
      // 提取情感分析和风险评估结果
      const emotionData = analysisResult.emotion_analysis
      ...
      if (riskData?.risk_level) {
        finalRiskLevel = riskData.risk_level
      } else if (riskData?.risk_score) {
        // 根据风险分数确定等级
        ...
        if (score >= 8) {
          ...
          finalRiskLevel = 'minimal'
        }
      }
      
      console.log('⚠️ 最终风险等级:', finalRiskLevel)
      ...
      console.error('❌ EasyBert分析失败，回退到关键词检测:', error)
      return {
        riskLevel: detectRiskLevelByKeywords(message)
      }
    }
  }

  // 保留关键词检测作为备用方案
  const detectRiskLevelByKeywords = (message: string): 'minimal' | 'low' | 'medium' | 'high' => {
  ...
  const sendMessage = () => {
    ...
    setTimeout(() => {
      const responses = {
        ...
        minimal: [
          '很高兴听到您的分享。您现在的状态听起来很不错。',
          '感谢您的信任。您的心态很积极，这对心理健康很有帮助。',
          '您的想法很健康。继续保持这样积极的心态，对您的整体福祉很有益。'
        ]
      }
