  ...
  const handleTextSubmit = async () => {
    ...
    try {
      ...
      try {
        ...
        // 当用户发送了6条消息后，就可以结束评估了
        if (conversationRounds >= 6 && !showCompletionTransition) {
          ...
          setTimeout(() => {
            ...
            setIsAIResponding(false)
            setCanManuallyComplete(true) // 允许手动完成
            ...
            // 使用最新的风险等级（已经在emotionRiskData中处理过）
            ...
            // 存储完成处理函数供手动触发使用
            ;(window as any).handleAssessmentCompletion = handleAssessmentCompletion
            
            // 缩短自动跳转时间到2秒，并添加提示
            setTimeout(() => {
              console.log('⏰ 2秒后自动跳转到评估结果，或点击任意位置立即跳转')
            }, 500)
            
            setTimeout(handleAssessmentCompletion, 2000)
          }, 500)
