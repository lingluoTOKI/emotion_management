...
export default function StudentAIAssessment() {
  ...
  const [canManuallyComplete, setCanManuallyComplete] = useState(false)
  ...
  const [currentConversationRounds, setCurrentConversationRounds] = useState<number>(0)
  ...
  const completionHandlerRef = useRef<(() => Promise<void>) | null>(null)
  ...
  const handleManualCompletion = useCallback(async () => {
    if (completionHandlerRef.current) {
      console.log('👆 用户手动触发评估完成')
      setCanManuallyComplete(false)
      await completionHandlerRef.current()
      completionHandlerRef.current = null // 清理引用
    }
  }, [])
  ...
  // 统一情绪处理管道
  const processEmotionPipeline = useCallback((emotionData: any, userInput: string, chatData: any) => {
    console.log('🔄 开始情绪处理管道:', { emotionData, userInput: userInput.slice(0, 50) })
    
    // 步骤1: 标准化EasyBert数据
    const normalizedAnalysis = emotionData ? {
    ...
    // 步骤2: 关键词验证和修正
    const correctedEmotion = normalizedAnalysis ? applyCorrectionKeywords(normalizedAnalysis, userInput) : null
    
    // 步骤3: 生成对话策略
    const dialogueStrategy = correctedEmotion ? getDialogueStrategy(correctedEmotion, userInput) : null
    
    // 步骤4: 计算最终风险等级
    const finalRiskLevel = calculateFinalRiskLevel(dialogueStrategy, chatData)
    
    // 步骤5: 映射中文显示
    const chineseEmotion = mapEmotionToChinese(correctedEmotion?.dominant_emotion || 'neutral')
    
    console.log('✅ 情绪处理管道完成:', {
      original: normalizedAnalysis?.dominant_emotion,
      corrected: correctedEmotion?.dominant_emotion,
      chinese: chineseEmotion,
      risk: finalRiskLevel
    })
    ...
    return {
      chineseEmotion,
      finalRiskLevel,
      correctedAnalysis: correctedEmotion,
      finalStrategy: dialogueStrategy,
      originalAnalysis: normalizedAnalysis
    }
  }, [])
  
  // 关键词修正函数
  const applyCorrectionKeywords = (analysis: any, userInput: string) => {
    const explicitEmotions = {
      '焦虑': ['焦虑', '紧张', '担心', '不安', '心慌'],
      '开心': ['开心', '高兴', '快乐', '愉快', '兴奋', '喜欢'],
      '悲伤': ['难过', '伤心', '沮丧', '痛苦'],
      '愤怒': ['生气', '愤怒', '烦躁', '气愤'],
      '恐惧': ['害怕', '恐惧', '怕', '畏惧']
    }
    
    for (const [emotion, keywords] of Object.entries(explicitEmotions)) {
      if (keywords.some(keyword => userInput.includes(keyword))) {
        const newEmotion = emotion === '焦虑' ? 'anxiety' : 
                          emotion === '开心' ? 'happiness' : 
                          emotion === '悲伤' ? 'sadness' : 
                          emotion === '愤怒' ? 'anger' : 'fear'
        
        console.log(`🎯 关键词修正: "${emotion}" -> ${newEmotion}`)
        return { ...analysis, dominant_emotion: newEmotion }
      }
    }
  ...
  // 风险等级计算
  const calculateFinalRiskLevel = (strategy: any, chatData: any) => {
    if (!strategy) {
      return chatData.risk_assessment?.risk_level || 'minimal'
    }
    
    const easyBertRisk = strategy.risk_level
    const backendRisk = chatData.risk_assessment?.risk_level || 'minimal'
    
    const riskPriority = (level: string) => {
      switch (level) {
        case 'high': return 4
    ...
    return riskPriority(easyBertRisk) >= riskPriority(backendRisk) ? easyBertRisk : backendRisk
  }
  
  // 情绪中文映射
  const mapEmotionToChinese = (emotion: string) => {
    ...
    return emotionMapping[emotion] || '平稳'
  }
  ...
  const getDialogueStrategy = (analysis: any, userInput: string) => {
    ...
    if (sentiment_score < -0.6 && emotion_intensity > 0.6) {
      ...
      // 积极情感分数：根据情绪类型和强度判断
      if (['happiness', 'joy', 'love', 'positive'].includes(analysis.dominant_emotion)) {
        risk_score = 1 // 积极情绪 = 极低风险，无论强度多高
      } else {
        risk_score = emotion_intensity > 0.7 ? 2 : 1 // 其他情况：低风险或极低风险
      }
    } else if (sentiment_score >= -0.3 && sentiment_score <= 0.3) {
      // 中性情感分数，但要考虑情绪强度和具体情绪类型
      if (analysis.dominant_emotion === 'sadness' && emotion_intensity > 0.6) {
        risk_score = 3 // 高强度悲伤 = 中等风险
      } else if (analysis.dominant_emotion === 'sadness' && emotion_intensity > 0.4) {
        risk_score = 3 // 中等强度悲伤 = 中等风险
      } else if (emotion_intensity > 0.7 && ['sadness', 'depression', 'anxiety', 'anger', 'fear'].includes(analysis.dominant_emotion)) {
        risk_score = 3 // 只有负面高强度情绪需要关注
      } else if (emotion_intensity > 0.7) {
        risk_score = 1 // 积极高强度情绪（如开心、兴奋）= 极低风险
      } else {
        risk_score = 2 // 一般情况 = 低风险
      }
    } else if (sentiment_score < -0.3) {
    ...
    const hasSocialIssues = socialIssueKeywords.some(keyword => userInput.includes(keyword))
    ...
    const hasPositive = positiveKeywords.some(keyword => userInput.includes(keyword))
    ...
    console.log('🎯 风险评估详情:', { 
      ...
      hasPositive, 
      risk_score, 
      final_risk: strategy.risk_level,
      evaluation_reason: risk_score >= 3 ? '中等或高风险' : risk_score >= 2 ? '低风险' : '极低风险'
    })
    ...
    try {
      ...
      try {
        ...
        const riskData = chatData.risk_assessment
                        ...
                        if (emotionData && (emotionData as any).analysis_method === 'bert') {
        ...
        // 使用统一的情绪处理管道
        const emotionRiskData = processEmotionPipeline(emotionData, inputContent, chatData)
        
        // 保留旧的processEmotionAndRisk函数定义以备用
        const processEmotionAndRisk = () => {
          let processedData = {
            chineseEmotion: '平稳',
            finalRiskLevel: 'minimal' as 'minimal' | 'low' | 'medium' | 'high',
            correctedAnalysis: null as any,
            finalStrategy: null as any
          }
          
          // 基于EasyBert分析结果更新情绪显示和风险等级
          // 注意：此函数已被processEmotionPipeline替代，这里保留仅作备用
          if (false) { // 禁用旧逻辑
            const analysis = null, strategy = null
            ...
            // 使用EasyBert的情绪识别，但进行关键词验证和修正
            let detectedEmotion = analysis.dominant_emotion
            
            // 关键词验证：如果用户明确表达了某种情绪，优先使用
            const explicitEmotions = {
              '焦虑': ['焦虑', '紧张', '担心', '不安', '心慌'],
              '开心': ['开心', '高兴', '快乐', '愉快', '兴奋', '喜欢'],
              '悲伤': ['难过', '伤心', '沮丧', '痛苦'],
            ...
            for (const [emotion, keywords] of Object.entries(explicitEmotions)) {
              if (keywords.some(keyword => inputContent.includes(keyword))) {
                console.log(`🎯 关键词修正：用户明确表达"${emotion}"，覆盖EasyBert识别的"${detectedEmotion}"`)
                detectedEmotion = emotion === '焦虑' ? 'anxiety' : 
                                 emotion === '开心' ? 'happiness' : 
                                 emotion === '悲伤' ? 'sadness' : 
                                 emotion === '愤怒' ? 'anger' : 'fear'
                break
            ...
            processedData.chineseEmotion = emotionMapping[detectedEmotion] || detectedEmotion
            console.log('🎨 EasyBert情绪分析结果:', analysis.dominant_emotion, '->', processedData.chineseEmotion)
            
            // 如果情绪被修正了，重新计算策略和风险等级
            let finalStrategy = strategy
            if (detectedEmotion !== analysis.dominant_emotion) {
              const correctedAnalysis = { ...analysis, dominant_emotion: detectedEmotion }
              finalStrategy = getDialogueStrategy(correctedAnalysis, inputContent)
              console.log('🔄 情绪修正后重新计算策略:', {
                original_emotion: analysis.dominant_emotion,
                corrected_emotion: detectedEmotion,
                chinese_emotion: processedData.chineseEmotion,
                original_risk: strategy.risk_level,
                corrected_risk: finalStrategy.risk_level
              })
            }
            console.log('🎯 最终对话策略:', finalStrategy)
            
            // 使用修正后的风险等级，如果后端也有风险评估则取更严重的
            const easyBertRiskLevel = finalStrategy.risk_level
            ...
            processedData.finalRiskLevel = (getRiskPriority(easyBertRiskLevel) >= getRiskPriority(backendRiskLevel) 
              ? easyBertRiskLevel 
              : backendRiskLevel) as 'minimal' | 'low' | 'medium' | 'high'
            
            console.log('⚠️ 最终风险等级:', { easyBert: easyBertRiskLevel, backend: backendRiskLevel, final: processedData.finalRiskLevel })
            
            // 存储修正后的分析结果
            processedData.correctedAnalysis = { ...analysis, dominant_emotion: detectedEmotion }
            processedData.finalStrategy = finalStrategy
            
          } else {
            console.log('❌ 没有收到EasyBert分析结果，尝试使用后端风险评估')
            
            // 如果没有EasyBert结果，但有后端风险评估，使用后端结果
            if (chatData.risk_assessment?.risk_level) {
              processedData.finalRiskLevel = chatData.risk_assessment.risk_level as 'minimal' | 'low' | 'medium' | 'high'
              console.log('🔄 使用后端风险评估:', processedData.finalRiskLevel)
            }
          }
        ...
        // 注意：现在使用processEmotionPipeline代替了processEmotionAndRisk
        
        // 统一更新状态（避免多次setState调用）
        setEmotionTrend(prev => {
          console.log('🔄 统一更新情绪状态:', {
            before: { emotion: prev.currentDominant, risk: prev.riskLevel },
            after: { emotion: emotionRiskData.chineseEmotion, risk: emotionRiskData.finalRiskLevel }
          })
          return {
            ...prev,
            currentDominant: emotionRiskData.chineseEmotion,
            riskLevel: emotionRiskData.finalRiskLevel
          }
        })
        
        // 更新评估进度
        if (emotionRiskData.correctedAnalysis && emotionRiskData.finalStrategy) {
          setAssessmentProgress(prev => ({
            ...prev,
            easyBertAnalysis: emotionRiskData.correctedAnalysis,
            dialogueStrategy: emotionRiskData.finalStrategy
          }))
        }
        
        // 统一危机检测处理
        const handleCrisisDetection = () => {
          const backendCrisisHandled = chatData.message.includes('您的生命非常宝贵') || 
                                     chatData.message.includes('立即寻求帮助') ||
                                     chatData.message.includes('全国24小时心理危机干预热线')
          
          if (!backendCrisisHandled && emotionRiskData.correctedAnalysis && emotionRiskData.finalStrategy) {
            checkEasyBertCrisisSignals(emotionRiskData.correctedAnalysis, emotionRiskData.finalStrategy, inputContent)
          } else if (backendCrisisHandled) {
            console.log('✅ 后端已处理危机情况，跳过EasyBert危机检测')
          }
        }
        ...
        const conversationRounds = totalUserMessages // 对话轮数基于用户发送的消息数
        
        // 更新当前对话轮数状态
        setCurrentConversationRounds(conversationRounds)
        ...
        const shouldCompleteAssessment = () => {
          // 检查完成条件
          const reachedRounds = conversationRounds >= 6
          const hasBackendRedirect = chatData.redirect_action?.type === 'complete_assessment'
          const alreadyShowing = showCompletionTransition
          
          return (reachedRounds || hasBackendRedirect) && !alreadyShowing
        }
        ...
        if (shouldCompleteAssessment()) {
          ...
          const completeAssessmentFlow = async () => {
            ...
            const aiAssessmentData = {
              ...
              easyBertAnalysis: emotionRiskData.correctedAnalysis || easyBertResult?.analysis,
              dialogueStrategy: emotionRiskData.finalStrategy || easyBertResult?.strategy,
              conversation_count: conversationRounds,
              completion_reason: chatData.redirect_action?.reason || 'conversation_complete',
          ...
          if (conversationRounds >= 6) {
            // 6轮对话完成 - 显示感谢消息和手动完成选项
            const thankYouMessage = "感谢您与我分享这么多内容！通过我们的对话，我已经对您的心理状态有了较为全面的了解。现在让我为您准备详细的评估结果。\n\n💡 提示：您可以点击任意位置立即查看评估结果，或等待2秒自动跳转。"
            
            setTimeout(() => {
              addAIMessage(thankYouMessage)
              setIsAIResponding(false)
              setCanManuallyComplete(true)
              
              // 存储完成函数到ref中
              completionHandlerRef.current = completeAssessmentFlow
              
              // 2秒后自动完成
              setTimeout(completeAssessmentFlow, 2000)
            }, 500)
          } else {
            // 后端指令完成 - 直接完成
            setTimeout(async () => {
              ...
              setTimeout(() => setShowManualRedirect(true), 1000)
            }, 800)
          }
        } else {
          // 正常的对话流程
          setTimeout(() => {
            ...
            // 统一危机处理逻辑
            const handleCrisisIfNeeded = () => {
              // 检查是否已经有危机干预（后端或EasyBert）
              const hasCrisisMessage = chatData.message.includes('您的生命非常宝贵') || 
                                     chatData.message.includes('立即寻求帮助') ||
                                     chatData.message.includes('全国24小时心理危机干预热线')
              
              // 检查是否是高风险评估结果
              const isHighRisk = emotionRiskData.finalRiskLevel === 'high'
              
              if (hasCrisisMessage || isHighRisk) {
                console.log('🚨 检测到危机情况或高风险，准备完成评估')
                
                // 确保风险等级为高
                if (!hasCrisisMessage && isHighRisk) {
                  // 只有在后端没有发送危机消息，但风险等级为高时，才发送前端危机消息
                ...
                // 危机情况下，3秒后自动完成评估
                setTimeout(async () => {
                  console.log('🚨 危机情况：3秒后自动完成评估')
                  await completeAssessment()
                  setShowCompletionTransition(true)
                }, 3000)
                
                return true // 表示已处理危机，不需要继续正常流程
              }
              
              return false // 表示没有危机，继续正常流程
            }
            
            // 执行危机检测，如果没有危机才继续正常对话
            const crisisHandled = handleCrisisIfNeeded()
        ...
        {currentStep === 'conversation' && !showCompletionTransition && (
          ...
          <div className="flex items-center justify-between">
              ...
              <div>
                      ...
                      <div className="flex items-center space-x-4 mt-2 text-sm">
                        ...
                        <span className="bg-white bg-opacity-20 px-2 py-1 rounded text-xs">
                          已评估: {Math.min(currentConversationRounds, assessmentProgress.totalQuestions)}/{assessmentProgress.totalQuestions}项
                        </span>
                      </div>
              </div>
            </div>
            ...
            <div className="flex items-center space-x-2">
              ...
              {/* 对话区域 */}
              <div 
                className={`h-96 overflow-y-auto p-6 space-y-4 ${canManuallyComplete ? 'cursor-pointer' : ''}`}
                onClick={() => {
            ...
            >
                ...
                {canManuallyComplete && (
                  <motion.div
                    ...
                    animate={{ opacity: 1, scale: 1 }}
                    className="absolute bottom-4 right-4 bg-blue-600 text-white px-4 py-2 rounded-lg shadow-lg cursor-pointer hover:bg-blue-700 transition-colors z-10"
                    onClick={handleManualCompletion}
                  >
                    <div className="flex items-center space-x-2">
                      <span>📋</span>
                      <span className="text-sm font-medium">立即查看评估结果</span>
                    </div>
                  </motion.div>
                )}
        </div>
