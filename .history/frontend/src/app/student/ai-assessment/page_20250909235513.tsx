        ...
        if (conversationRounds >= 6 && !showCompletionTransition) {
          ...
          setTimeout(() => {
            ...
            if (hasSuicidalThoughts) {
              console.log('🚨 前端检测到自杀风险，检查后端是否已处理')
              
              // 检查后端是否已经返回了危机干预消息
              const backendHandledCrisis = (chatData.risk_assessment?.risk_level === 'high' && 
                                          chatData.message.includes('全国24小时心理危机干预热线')) ||
                                         chatData.message.includes('您的生命非常宝贵') ||
                                         chatData.message.includes('立即寻求帮助')
              
              if (!backendHandledCrisis) {
                // 只有在后端没有处理时，才发送前端的危机干预消息
                setTimeout(() => {
                ...
                console.log('✅ 后端已处理危机情况，跳过前端重复处理')
                // 后端已经处理了，标记风险等级并准备结束评估
                setEmotionTrend(prev => ({
                  ...prev,
                  riskLevel: 'high'
                }))
              ...
              if (!hasCrisisMessage) {
                ...
                console.log('⚠️ 检测到危机干预消息，跳过生成下一个问题')
              }
            }
          }, 800)
        }
