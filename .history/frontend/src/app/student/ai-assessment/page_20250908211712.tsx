      ...
      try {
        ...
        if (chatData.redirect_action && chatData.redirect_action.type === 'complete_assessment' && !showCompletionTransition) {
          ...
          setTimeout(() => {
            ...
            // 检查是否达到6轮对话，如果是则完成评估
            const userMessages = messages.filter(m => m.sender === 'user').length + 1 // +1 因为当前消息还没加入messages
            ...
            if (userMessages >= 6 && !showCompletionTransition) {
              ...
              setTimeout(async () => {
                ...
                // 添加完成消息
                const completionMessage = `🎉 **恭喜您完成了AI智能心理评估！**\n\n感谢您与我进行了${userMessages}轮深入的心理对话。通过我们的交流，我已经收集到了足够的信息来初步了解您的心理状态。\n\n🧠 **AI分析结果：**\n• 当前情绪状态：${emotionTrend.currentDominant}\n• 风险等级：${emotionTrend.riskLevel === 'high' ? '高风险' : emotionTrend.riskLevel === 'medium' ? '中等风险' : emotionTrend.riskLevel === 'low' ? '低风险' : '极低风险'}\n• EasyBert情感分析：已完成\n\n为了给您提供更准确和全面的心理健康评估，接下来需要您完成一些标准化的心理量表。这些量表将与我们的AI对话分析结果相结合，为您生成专业的综合评估报告。\n\n📋 **准备好继续了吗？** 点击下方按钮进入传统量表评估阶段！`
            ...
            >
              ...
              <div className="flex justify-center space-x-3">
                <button
                  ...
                  className="px-6 py-2 bg-gradient-to-r from-blue-600 to-purple-600 text-white rounded-lg hover:from-blue-700 hover:to-purple-700 transition-all duration-200 font-medium flex items-center space-x-2"
                >
