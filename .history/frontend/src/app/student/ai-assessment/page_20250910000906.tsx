...
export default function StudentAIAssessment() {
  ...
  // 获取阶段显示名称（优化版）
  const getPhaseDisplayName = (phase: string, questionCount: number) => {
    if (questionCount >= 6) {
      return '即将完成'
    }
  ...
  // 合并风险等级：优先使用更高的风险等级
  const mergeRiskLevels = (level1: string, level2: string): 'minimal' | 'low' | 'medium' | 'high' => {
    const riskPriority = {
      'high': 4,
      'medium': 3,
      'low': 2,
    ...
    const priority1 = riskPriority[level1 as keyof typeof riskPriority] || 1
    const priority2 = riskPriority[level2 as keyof typeof riskPriority] || 1
    
    return priority1 >= priority2 ? level1 as 'minimal' | 'low' | 'medium' | 'high' : level2 as 'minimal' | 'low' | 'medium' | 'high'
  }

  // 检测消息中是否已包含危机干预内容
  const hasCrisisInterventionInMessage = (message: string): boolean => {
...
记住，困难是暂时的，您有能力度过这个阶段。`
  ...
  const resetAssessmentState = () => {
    ...
    setCurrentConversationRounds(0) // 重置对话轮数
    setCanManuallyComplete(false)   // 重置手动完成状态
    ...
    try {
      ...
      try {
        ...
        if (easyBertResult && easyBertResult.analysis) {
          ...
          chineseEmotion = emotionMapping[detectedEmotion] || detectedEmotion
          ...
          if (detectedEmotion !== analysis.dominant_emotion) {
            ...
            console.log('🔄 情绪修正后重新计算策略:', {
              ...
              chinese_emotion: chineseEmotion,
          ...
          // 合并风险等级评估：优先使用更高的风险等级
          finalRiskLevel = mergeRiskLevels(finalStrategy.risk_level, chatData.risk_assessment?.risk_level || 'minimal')
          
          console.log('⚠️ 最终风险等级:', { easyBert: finalStrategy.risk_level, backend: chatData.risk_assessment?.risk_level, final: finalRiskLevel })
          ...
          setEmotionTrend(prev => {
            ...
            console.log('✅ EasyBert状态更新完成:', newState)
            return newState
          })
          
          // 存储修正后的EasyBert分析结果和对话策略
          const correctedAnalysis = { ...analysis, dominant_emotion: detectedEmotion }
          setAssessmentProgress(prev => ({
            ...prev,
            easyBertAnalysis: correctedAnalysis,
            dialogueStrategy: finalStrategy
          }))
          
          // 统一危机检测：避免重复干预
          if (!hasCrisisInterventionInMessage(chatData.message)) {
            checkEasyBertCrisisSignals(correctedAnalysis, finalStrategy, inputContent)
          } else {
          ...
          if (chatData.risk_assessment?.risk_level) {
            const backendRiskLevel = chatData.risk_assessment.risk_level
            console.log('🔄 使用后端风险评估:', backendRiskLevel)
            
            setEmotionTrend(prev => {
              console.log('🔄 基于后端风险评估更新状态:', {
                before: { emotion: prev.currentDominant, risk: prev.riskLevel },
                after: { emotion: prev.currentDominant, risk: backendRiskLevel }
              })
              const newState = {
                ...prev,
                riskLevel: backendRiskLevel as 'minimal' | 'low' | 'medium' | 'high'
              }
              console.log('✅ 后端风险评估状态更新完成:', newState)
              return newState
            })
          }
        }
        ...
        // 计算实际对话轮数：基于已发送的用户消息数（当前消息已经添加到messages中）
        const updatedMessages = [...messages, userMessage] // 模拟更新后的messages状态
        const totalUserMessages = updatedMessages.filter(m => m.type === 'user').length
        ...
        setIsAIResponding(false)
        // 如果AI调用失败，使用备用回复，但不要额外生成问题
        setTimeout(() => {
          addAIMessage('抱歉，我遇到了一些技术问题。请您重新说一遍，或者分享更多您的感受吧。')
        }, 1500)
      }
    } catch (error) {
              ...
              <div>
                      ...
                      <div className="flex items-center space-x-4 mt-2 text-sm">
                        ...
                        <span className="bg-white bg-opacity-20 px-2 py-1 rounded text-xs">
                          已评估: {assessmentProgress.questionCount}/{assessmentProgress.totalQuestions}项
                        </span>
                      </div>
              </div>
            </div>
