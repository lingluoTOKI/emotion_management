...
export default function StudentAIAssessment() {
  ...
  const checkEasyBertCrisisSignals = (analysis: any, strategy: any, userInput: string) => {
    ...
    if (crisisScore >= 70) {
      ...
      // 轻度提醒
      triggerCrisisIntervention('mild', analysis, strategy, crisisReason, crisisScore)
    }
  ...
  const triggerCrisisIntervention = (
    level: 'emergency' | 'high' | 'moderate' | 'mild',
    analysis: any,
    strategy: any,
    reasons: string[],
    score: number
...
**💙 当前分析结果：**
• 情绪状态：${analysis.dominant_emotion} (${(analysis.emotion_intensity * 100).toFixed(0)}%强度)
• 情感倾向：${analysis.sentiment_score.toFixed(2)}
• 风险评估：${strategy.risk_level}
...
**📊 当前情况分析：**
• 主要情绪：${analysis.dominant_emotion}
• 情绪强度：${(analysis.emotion_intensity * 100).toFixed(0)}%
• 整体风险：${strategy.risk_level}
...
**📈 情绪分析：**
• 当前情绪：${analysis.dominant_emotion} 
• 情感程度：${analysis.sentiment_score.toFixed(2)}

记住，困难是暂时的，您有能力度过这个阶段。`
        break

      case 'mild':
        interventionType = '积极关注'
        interventionMessage = `🌟 **感谢您的开放分享**

我注意到您在表达一些复杂的情感。能够诚实地面对和表达自己的感受是很有价值的。
    ...
    try {
      ...
      try {
        ...
        console.log('🔢 对话轮数检查:', {
          ...
          用户消息数: messages.filter(m => m.type === 'user').length,
          计算的用户消息数: userMessages,
          是否显示完成界面: showCompletionTransition,
          是否达到6轮: userMessages >= 6,
          后端redirect_action: chatData.redirect_action
        })
        
        if (userMessages >= 6 && !showCompletionTransition) {
          // 达到6轮对话，先发送感谢消息，然后立即跳转
          const thankYouMessage = "谢谢您的详细分享！通过我们的对话，我已经充分了解了您的心理状态。现在让我为您准备评估结果。"
          ...
          setTimeout(() => {
            ...
            if (hasSuicidalThoughts) {
              // 立即发送关怀回应，不生成下一个问题
              setTimeout(() => {
                const crisisResponse = "我非常担心您刚才提到的想法。您的生命对我们来说很重要，请不要独自承受这种痛苦。建议您立即寻求专业帮助：\n\n🆘 **紧急求助热线：**\n• 全国心理危机干预热线：400-161-9995\n• 北京危机干预热线：400-161-9995\n• 上海精神卫生中心：021-64387250\n\n如果您现在感到危险，请立即拨打120或前往最近的医院急诊科。您并不孤单，有很多人愿意帮助您。"
