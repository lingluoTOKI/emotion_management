  ...
  const handleTextSubmit = async () => {
    ...
    try {
      ...
      try {
        ...
        // 检查是否有跳转指令 - 只在第一次触发时执行
        if (chatData.redirect_action && chatData.redirect_action.type === 'complete_assessment' && !showCompletionTransition) {
          // AI发送了评估完成指令，直接完成评估
          setTimeout(async () => {
        ...
        } else if (!chatData.redirect_action || chatData.redirect_action.type !== 'complete_assessment') {
          // 检查是否达到6轮对话，如果是则立即完成评估
          const userMessages = messages.filter(m => m.type === 'user').length + 1 // +1 因为当前消息还没加入messages
          
          if (userMessages >= 6 && !showCompletionTransition) {
            // 达到6轮对话，立即完成评估，不显示AI响应
            setTimeout(async () => {
              setIsAIResponding(false)
              ...
              const aiAssessmentData = {
                ...
                easyBertAnalysis: easyBertResult?.analysis,
                dialogueStrategy: easyBertResult?.strategy,
                conversation_count: userMessages,
          ...
          <div className="max-w-2xl mx-auto">
            ...
            >
              ...
              <div className="space-y-4">
                <h3 className="text-center font-semibold text-gray-900">🎯 下一步选择</h3>
                <div className="grid grid-cols-1 gap-3">
                  ...
                  <button
                    ...
                    className="w-full px-6 py-3 bg-gradient-to-r from-blue-600 to-purple-600 text-white rounded-lg hover:from-blue-700 hover:to-purple-700 transition-all duration-200 font-medium flex items-center justify-center space-x-2 shadow-md"
                  >
                    <FileText className="w-4 h-4" />
                    <span>继续传统量表评估</span>
                    <span className="text-xs bg-white bg-opacity-20 px-2 py-1 rounded">推荐</span>
                  </button>
                  ...
                  <button
                    ...
                    className="w-full px-6 py-3 bg-gradient-to-r from-green-600 to-teal-600 text-white rounded-lg hover:from-green-700 hover:to-teal-700 transition-all duration-200 font-medium flex items-center justify-center space-x-2 shadow-md"
                  >
                  ...
                  <button
                    ...
                    className="w-full px-4 py-2 border border-gray-300 text-gray-600 rounded-lg hover:bg-gray-50 transition-colors flex items-center justify-center space-x-2"
                  >
                ...
                <div className="bg-gray-50 rounded-lg p-3 text-xs text-gray-600 space-y-1">
                  <p><strong>传统量表：</strong>通过标准化问卷获得更精确的心理健康评估</p>
                  <p><strong>AI心理辅导：</strong>基于评估结果，获得个性化的心理支持和建议</p>
                  <p><strong>返回仪表板：</strong>查看历史评估记录和其他功能</p>
                </div>
              </div>
            </motion.div>
          </div>
