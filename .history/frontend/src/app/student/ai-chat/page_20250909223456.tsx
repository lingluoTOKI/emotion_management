...
请告诉我，今天您希望我们聊什么呢？`
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      // 获取EasyBert情感分析结果
      const emotionAnalysis = chatData.emotion_analysis
      const riskAssessment = chatData.risk_assessment
      console.log('🧠 收到EasyBert情感分析:', emotionAnalysis)
      console.log('⚠️ 收到风险评估:', riskAssessment)
                    ...
                    `}>
                      <div className="flex items-start space-x-2">
                        ...
                        <div className="flex-1">
                          ...
                          {/* 显示EasyBert情感分析结果 */}
                          {message.type === 'ai' && message.emotion_analysis && (
                            <div className="mt-3 p-3 bg-blue-50 rounded-lg border-l-4 border-blue-400">
                              <div className="text-xs font-semibold text-blue-800 mb-2">🧠 EasyBert情感分析结果</div>
                              <div className="text-xs text-blue-700 space-y-1">
                                <div className="flex justify-between">
                                  <span>主导情绪:</span>
                                  <span className="font-medium">
                                    {message.emotion_analysis.dominant_emotion === 'sadness' ? '悲伤' :
                                     message.emotion_analysis.dominant_emotion === 'happiness' ? '开心' :
                                     message.emotion_analysis.dominant_emotion === 'anxiety' ? '焦虑' :
                                     message.emotion_analysis.dominant_emotion === 'anger' ? '愤怒' :
                                     message.emotion_analysis.dominant_emotion === 'neutral' ? '平稳' :
                                     message.emotion_analysis.dominant_emotion}
                                  </span>
                                </div>
                                <div className="flex justify-between">
                                  <span>置信度:</span>
                                  <span className="font-medium">
                                    {(message.emotion_analysis.confidence * 100).toFixed(1)}%
                                  </span>
                                </div>
                                <div className="flex justify-between">
                                  <span>分析方法:</span>
                                  <span className="font-medium">
                                    {message.emotion_analysis.analysis_method === 'bert' ? 'BERT模型' :
                                     message.emotion_analysis.analysis_method === 'keyword_fallback' ? '关键词分析' :
                                     message.emotion_analysis.analysis_method}
                                  </span>
                                </div>
                              </div>
                            </div>
                          ...
                          {/* 显示风险评估结果 */}
                          {message.type === 'ai' && message.risk_assessment && (
                            <div className="mt-2 p-3 bg-yellow-50 rounded-lg border-l-4 border-yellow-400">
                              <div className="text-xs font-semibold text-yellow-800 mb-2">⚠️ 风险评估</div>
                              <div className="text-xs text-yellow-700 space-y-1">
                                <div className="flex justify-between">
                                  <span>风险等级:</span>
                                  <span className={`font-medium ${
                                    message.risk_assessment.risk_level === 'high' ? 'text-red-600' :
                                    message.risk_assessment.risk_level === 'medium' ? 'text-yellow-600' :
                                    'text-green-600'
                                  }`}>
                                    {message.risk_assessment.risk_level === 'high' ? '高风险' :
                                     message.risk_assessment.risk_level === 'medium' ? '中等风险' :
                                     message.risk_assessment.risk_level === 'low' ? '低风险' : '极低风险'}
                                  </span>
                                </div>
                                {message.risk_assessment.risk_score && (
                                  <div className="flex justify-between">
                                    <span>风险评分:</span>
                                    <span className="font-medium">{message.risk_assessment.risk_score}</span>
                                  </div>
                                )}
                              </div>
                            </div>
